...
- (void)layoutViewsWithKeyboardHeight:(CGFloat)keboardHeight
{
    ...
    _clearTextBtn.frame = CGRectMake(self.view.width - CELL_PADDING_4 - CLEAR_BTN_WIDTH,
                                     _postBtnBar.top - CLEAR_BTN_WIDTH,
    ...
    switch (selectedIndex) {
            ...
            // 表情
        {
            if (TTIsKeyboardVisible()) {
                ...
                // 显示表情选择框
                if (!_emotionMainView) {
                    _emotionMainView = [[OSCEmotionMainView alloc] initWithFrame:CGRectZero];
...
{
    _emotionMainView.frame = _inputBackgroundImageView.frame;
...
- (void)imagePickerControllerDidCancel:(UIImagePickerController *)picker
{
    ...
    // 如果表情没有显示，则自动弹出键盘，实现比较丑陋，后面考虑简化
    if (!_emotionMainView.superview) {
        [_statusesTextView becomeFirstResponder];
    }
}
